<div class="skill-element fader @if (isset($zeroOpacity)) {{ 'zeroOpacity' }} @endif">
    <div class="input-group col-sm-4">
        ...
        {{ Form::text('categorySkills['. $categoryId .'][skillName][]', '', ['class' =>'form-control skill-input', 'autocomplete' => 'off', 'data-provide' => 'typeahead', 'maxlength' => 20, 'placeholder' => trans('messages.skill.name_placeholder')]) }}
    </div>
    <div class="input-group col-sm-5">
        <span class="input-group-addon">{{ Form::label('skill_year', trans('messages.skill.year'), ['class' =>'skill_year']) }}</span>
        {{ Form::select('categorySkills['. $categoryId .'][skillYear][]', App\Services\ProfileService::getSkillYearOptions(), 0.5, ['class' => 'form-control']) }}
    </div>
