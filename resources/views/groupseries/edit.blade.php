...
<div class="series-new">
    ...
    @endif
    {{ Form::open(['action' => ['GroupSeriesController@update', 'encryptedGroupId' => $encryptedGroupId, 'groupSeriesId' => $groupSeries->id],
        'method' => 'POST','role' => 'form', 'files' => true,
        'id' => 'update-series', 'class' => 'serive']) }}
        <div class="col-lg-12 series-text">
            {{ Form::text('name', $groupSeries->name, ['id' => 'name', 'class' => 'form-control write',
            ...
            {{ Form::select('language_code', Config::get('detect_language.code'),
                $groupSeries->language_code, ['id' => 'language_code', 'data-type' => 'default']) }}<br/><br/>
            {{ Form::textarea('description', $groupSeries->description, ['id' => 'description', 'class' => 'form-control',
                'placeholder' => trans('labels.group_series.series_description')]) }}
        </div>
        <div class="clear-both"></div>
        <div class="group-series-list form-group sortable">
            {{ \App\Services\HelperService::getSeriesListItemsToEdit($groupSeries->id, false) }}
        </div>
        ...
        <div class="submit">
            <input class="save" data-form-type="edit" type="button" value="{{ trans('buttons.save') }}" />
        </div>
    {{ Form::close() }}
</div>
...
@section('group-script')
    ...
    {{ HTML::script(version('js_min/groupseries_edit.min.js')) }}
@stop
