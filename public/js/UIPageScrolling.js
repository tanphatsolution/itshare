    ...
    $.fn.UIPageScrolling = function (settings) {
        ...
        function processMouseWheel(e) {
            var event = window.event || e.originalEvent,
                value = event.wheelDelta || (-120) * event.detail,
                delta = Math.max(-1, Math.min(1, value));
            timeStamp.curTime = e.timeStamp - timeStamp.preTime;
            timeStamp.preTime  = e.timeStamp;
            if(timeStamp.clock < 50) {
                canScroll = false;
            }
            if(timeStamp.clock > 150) {
                canScroll = true;
            }
